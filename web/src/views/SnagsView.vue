...
async function resolveSnag(r: unknown) {
  const id = String((r as any).id ?? '')
  const rows = [...items.value]
  const idx = rows.findIndex((x: any) => String(x.id ?? '') === id)
  if (idx < 0) return
  rows[idx] = { ...(rows[idx] as any), status: 'Resolved' }
  const res = await api.sync({ snags: rows })
  if (res.ok) {
    showToast(`Snag ${id} resolved`)
...
onMounted(async () => {
  ...
  if (r.ok && r.data) {
    const arr = r.data.collections['snags']
...
const stats = computed(() => [
  { label: 'Snags', value: String(rows.value.length), color: '#2f80ed' },
  { label: 'Open', value: String(rows.value.filter((s: any) => s.status === 'Open' || s.status === 'Pending').length), color: '#e65100' },
  { label: 'Resolved', value: String(rows.value.filter((s: any) => s.status === 'Resolved' || s.status === 'Fixed').length), color: '#2e7d32' }
])
...
const columns = computed<TableColumn<any>[]>(() => [
  {
    ...
    label: 'ID',
    sortable: true,
    renderHtml: (x) => `<div style='font-weight:600;color:#2f80ed'>${esc(x.id)}</div><div style='font-size:9px;color:#888'>${esc(x.handoverId||'')}</div>`
  },
  {
    key: 'item',
    label: 'Item',
    sortable: false,
    renderHtml: (x) => `<div style='font-weight:500;color:#333'>${esc(x.item)}</div><div style='font-size:9px;color:#888'>${esc(x.category||'')} · ${esc(x.unit||'')}</div>`
  },
  {
    key: 'customer',
    label: 'Customer',
...
const actions = computed(() => [
  { label: '✓ Resolve', icon: '✅', show: (r: any) => !['Resolved', 'Fixed'].includes(String(r.status ?? '')), onClick: (r: unknown) => resolveSnag(r) },
...
<template>
  <div class="fade-in">
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px">
      <span class="page-title">Snags & Handover</span>
    ...
    <DataTable
      ...
      search-placeholder="Search snags…"
    />
  </div>
    <GenericDetailDrawer :record="detailRec" :title="'Snags & Handover'" @close="detailRec = null" :records="detailList" />
</template>
