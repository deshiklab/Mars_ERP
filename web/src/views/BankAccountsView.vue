...
onMounted(async () => {
  ...
  if (r.ok && r.data) {
    const arr = r.data.collections['bank_accounts']
...
const stats = computed(() => [
  ...
  { label: 'Total Balance', value: String(bdt(rows.value.reduce((s: number, b: any) => s + parseFloat(String(b.balance||'0').replace(/[^0-9.]/g,'')||'0'), 0))), color: '#2e7d32' },
  { label: 'Accounts (types)', value: String(new Set(rows.value.map((b: any) => b.type)).size), color: '#1565c0' }
])
...
const columns = computed<TableColumn<any>[]>(() => [
  {
    ...
    label: 'Bank',
    sortable: false,
    renderHtml: (x) => `<div style='font-weight:500;color:#333'>${esc(x.name)}</div><div style='font-size:9px;color:#888'>${esc(x.branch||'')}</div>`
  },
  {
    key: 'account',
    label: 'Account',
    sortable: false,
    renderHtml: (x) => `<span style='font-size:10px;color:#555'>${esc(x.account||'—')}</span>`
  },
  {
    key: 'type',
    label: 'Type',
  ...
  },
  {
    ...
    label: 'Balance',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:11px;color:#2e7d32;font-weight:700'>${esc(x.balance||'৳ 0')}</span>`
  },])
...
<template>
  <div class="fade-in">
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px">
      <span class="page-title">Bank Accounts</span>
    ...
    <GenericDetailDrawer :record="detailRec" :title="'Bank Accounts'" @close="detailRec = null" :records="detailList" />
</template>
