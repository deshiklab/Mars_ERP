...
onMounted(async () => {
  ...
  if (r.ok && r.data) {
    const arr = r.data.collections['reminder_log']
...
const stats = computed(() => [
  { label: 'Reminders', value: String(rows.value.length), color: '#2f80ed' },
  { label: 'Sent', value: String(rows.value.filter((r: any) => r.status === 'Sent').length), color: '#2e7d32' },
  { label: 'Failed', value: String(rows.value.filter((r: any) => r.status === 'Failed').length), color: '#c62828' }
])
...
const columns = computed<TableColumn<any>[]>(() => [
  ...
  },
  {
    ...
    label: 'Contact',
    sortable: false,
    renderHtml: (x) => `<div style='font-weight:500;color:#333'>${esc(x.contact)}</div><div style='font-size:9px;color:#888'>${esc(x.template||'')}</div>`
  },
  {
    key: 'stage',
    label: 'Stage',
  ...
  },
  {
    ...
    label: 'Channel',
    sortable: false,
    renderHtml: (x) => `<span style='font-size:10px;color:#555'>${esc(x.channel||'—')}</span>`
  },
  {
    key: 'date',
    label: 'Date',
...
<template>
  <div class="fade-in">
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px">
      <span class="page-title">Reminder Log</span>
    ...
    <GenericDetailDrawer :record="detailRec" :title="'Reminder Log'" @close="detailRec = null" :records="detailList" />
</template>
