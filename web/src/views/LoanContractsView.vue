...
async function setCloseStatus(r: unknown, st: string) {
  ...
  try {
    ...
    const res = await api.sync({ loan_contracts: rows })
...
onMounted(async () => {
  ...
  if (r.ok && r.data) {
    const arr = r.data.collections['loan_contracts']
...
const stats = computed(() => [
  { label: 'Contracts', value: String(rows.value.length), color: '#2f80ed' },
  { label: 'Active', value: String(rows.value.filter((l: any) => l.status === 'Active').length), color: '#2e7d32' },
  { label: 'Total', value: String(bdt(rows.value.reduce((s: number, l: any) => s + (l.amount ?? 0), 0))), color: '#1565c0' }
])
...
const columns = computed<TableColumn<any>[]>(() => [
  {
    ...
    label: 'Contract',
    sortable: false,
    renderHtml: (x) => `<div style='font-weight:500;color:#333'>${esc(x.title)}</div><div style='font-size:9px;color:#888'>${esc(x.refType||'')} ${esc(x.refId||'')}</div>`
  },
  {
    key: 'party',
    label: 'Party',
    sortable: false,
    renderHtml: (x) => `<div style='font-weight:500;color:#333'>${esc(x.party)}</div><div style='font-size:9px;color:#888'>${esc(x.signedBy||'')}</div>`
  },
  {
    key: 'amount',
    label: 'Amount',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:11px;color:#333;font-weight:700'>${bdt(x.amount)}</span>`
  },
  {
    key: 'startDate',
    label: 'Start',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:10px;color:#555'>${esc(x.startDate||'—')}</span>`
  },
  {
    key: 'endDate',
    label: 'End',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:10px;color:${x.endDate < '2026-08-10' ? '#c62828' : '#555'}'>${esc(x.endDate||'—')}</span>`
  },
  {
    key: 'status',
    label: 'Status',
...
<template>
  <div class="fade-in">
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px">
      <span class="page-title">Loan Contracts</span>
    ...
    <GenericDetailDrawer :record="detailRec" :title="'Loan Contracts'" @close="detailRec = null" :records="detailList" />
</template>
