...
onMounted(async () => {
  ...
  if (r.ok && r.data) {
    const arr = r.data.collections['payroll']
...
const stats = computed(() => [
  { label: 'Payroll Entries', value: String(rows.value.length), color: '#2f80ed' },
  { label: 'Paid', value: String(rows.value.filter((p: any) => p.status === 'Paid').length), color: '#2e7d32' },
  { label: 'Net Total', value: String(bdt(rows.value.reduce((s: number, p: any) => s + (p.netPay ?? 0), 0))), color: '#1565c0' }
])
...
const columns = computed<TableColumn<any>[]>(() => [
  {
    ...
    label: 'Worker',
    sortable: false,
    renderHtml: (x) => `<div style='font-weight:500;color:#333'>${esc(x.workerName)}</div><div style='font-size:9px;color:#888'>${esc(x.workerId||'')}</div>`
  },
  {
    key: 'month',
    label: 'Month',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:10px;color:#555'>${esc(x.month||'')} ${esc(x.year||'')}</span>`
  },
  {
    key: 'daysWorked',
    label: 'Days',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:10px;color:#333'>${esc(x.daysWorked??'—')}</span>`
  },
  {
    key: 'grossPay',
    label: 'Gross',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:10px;color:#333;font-weight:600'>${bdt(x.grossPay)}</span>`
  },
  {
    key: 'deductions',
    label: 'Deductions',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:10px;color:#c62828'>-${bdt(x.deductions)}</span>`
  },
  {
    key: 'netPay',
    label: 'Net',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:11px;color:#2e7d32;font-weight:700'>${bdt(x.netPay)}</span>`
  },
  {
    key: 'status',
    label: 'Status',
...
<template>
  <div class="fade-in">
    ...
    <GenericDetailDrawer :record="detailRec" :title="'Payroll'" @close="detailRec = null" :records="detailList" />
</template>
