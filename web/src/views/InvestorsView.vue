...
async function setCloseStatus(r: unknown, st: string) {
  ...
  try {
    ...
    const res = await api.sync({ investors: rows })
...
onMounted(async () => {
  ...
  if (r.ok && r.data) {
    const arr = r.data.collections['investors']
...
const stats = computed(() => [
  { label: 'Investors', value: String(rows.value.length), color: '#2f80ed' },
  { label: 'Total Invested', value: String(bdt(rows.value.reduce((s: number, i: any) => s + (i.totalInvested ?? 0), 0))), color: '#2e7d32' },
  { label: 'Active', value: String(rows.value.filter((i: any) => i.status === 'Active').length), color: '#1565c0' }
])
...
const columns = computed<TableColumn<any>[]>(() => [
  ...
  },
  {
    ...
    label: 'Invested',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:11px;color:#2e7d32;font-weight:700'>${bdt(x.totalInvested)}</span>`
  },
  {
    key: 'joinDate',
    label: 'Joined',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:10px;color:#555'>${esc(x.joinDate||'—')}</span>`
  },
  {
    key: 'status',
    label: 'Status',
...
const actions = computed(() => [
  { label: '✓ Close', icon: '✅', show: (r: any) => ['Active','Accepted'].includes(r.status), onClick: (r: any) => setCloseStatus(r, 'Inactive') },
...
<template>
  <div class="fade-in">
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px">
      <span class="page-title">Investors</span>
    ...
    <GenericDetailDrawer :record="detailRec" :title="'Investors'" @close="detailRec = null" :records="detailList" />
</template>
