...
onMounted(async () => {
  ...
  if (r.ok && r.data) {
    const arr = r.data.collections['knowledge_base']
...
const stats = computed(() => [
  { label: 'Articles', value: String(rows.value.length), color: '#2f80ed' },
  { label: 'Published', value: String(rows.value.filter((k: any) => k.status === 'Published').length), color: '#2e7d32' },
  { label: 'Total Views', value: String(rows.value.reduce((s: number, k: any) => s + (k.views ?? 0), 0)), color: '#1565c0' }
])
...
const columns = computed<TableColumn<any>[]>(() => [
  {
    ...
    label: 'Article',
    sortable: false,
    renderHtml: (x) => `<div style='font-weight:500;color:#333'>${esc(x.title)}</div><div style='font-size:9px;color:#888;max-width:300px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap'>${esc(x.content||'')}</div>`
  },
  {
    key: 'category',
    label: 'Category',
  ...
  },
  {
    ...
    label: 'Author',
    sortable: false,
    renderHtml: (x) => `<span style='font-size:10px;color:#555'>${esc(x.author||'—')}</span>`
  },
  {
    key: 'tags',
    label: 'Tags',
    sortable: false,
    renderHtml: (x) => `<span style='display:inline-flex;gap:3px;flex-wrap:wrap'>${(x.tags||[]).map((t: string) => `<span style='background:#f5f5f5;color:#666;border-radius:6px;padding:0 5px;font-size:8px'>${esc(t)}</span>`).join('')}</span>`
  },
  {
    key: 'views',
    label: 'Views',
...
<template>
  <div class="fade-in">
    ...
    <GenericDetailDrawer :record="detailRec" :title="'Knowledge Base'" @close="detailRec = null" :records="detailList" />
</template>
