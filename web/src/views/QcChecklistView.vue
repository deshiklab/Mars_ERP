...
async function setCompleteStatus(r: unknown, st: string) {
  ...
  try {
    ...
    const res = await api.sync({ qcChecklist: rows })
...
onMounted(async () => {
  ...
  if (r.ok && r.data) {
    const arr = r.data.collections['qcChecklist']
...
const stats = computed(() => [
  ...
  { label: 'Passed', value: String(rows.value.filter((q: any) => q.status === 'Passed').length), color: '#2e7d32' },
  { label: 'Failed', value: String(rows.value.filter((q: any) => q.status === 'Failed').length), color: '#c62828' }
])
...
const columns = computed<TableColumn<any>[]>(() => [
  ...
  },
  {
    ...
    label: 'Check Item',
    sortable: false,
    renderHtml: (x) => `<div style='font-weight:500;color:#333'>${esc(x.item)}</div><div style='font-size:9px;color:#888'>${esc(x.stage||'')} · ${esc(x.project||'')}</div>`
  },
  {
    key: 'standard',
    label: 'Standard',
    sortable: false,
    renderHtml: (x) => `<span style='font-size:10px;color:#555'>${esc(x.standard||'—')}</span>`
  },
  {
    key: 'checkedBy',
    label: 'Checked By',
    sortable: false,
    renderHtml: (x) => `<span style='font-size:10px;color:#555'>${esc(x.checkedBy||'—')}</span>`
  },
  {
    key: 'date',
    label: 'Date',
...
<template>
  <div class="fade-in">
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px">
      <span class="page-title">QC Checklist</span>
