...
async function markPaid(r: unknown) {
  const id = String((r as any).id ?? '')
  const rows = [...items.value]
  const idx = rows.findIndex((x: any) => String(x.id ?? '') === id)
  if (idx < 0) return
  rows[idx] = { ...(rows[idx] as any), status: 'Paid' }
  const res = await api.sync({ contractorPayments: rows })
  if (res.ok) {
    showToast(`Payment ${id} marked paid`)
...
onMounted(async () => {
  ...
  if (r.ok && r.data) {
    const arr = r.data.collections['contractorPayments']
...
const stats = computed(() => [
  ...
  { label: 'Total', value: String(bdt(rows.value.reduce((s: number, p: any) => s + (p.amount ?? 0), 0))), color: '#c62828' },
...
const columns = computed<TableColumn<any>[]>(() => [
  ...
  },
  {
    ...
    label: 'Contractor',
    sortable: false,
    renderHtml: (x) => `<div style='font-weight:500;color:#333'>${esc(x.contractor)}</div><div style='font-size:9px;color:#888'>${esc(x.workOrder||'')}</div>`
  },
  {
    key: 'type',
    label: 'Type',
...
const actions = computed(() => [
  { label: '✓ Mark Paid', icon: '💰', show: (r: any) => String(r.status ?? '') !== 'Paid', onClick: (r: unknown) => markPaid(r) },
  { label: '✓ Mark Paid', icon: '💰', show: (r: any) => String(r.status ?? '') !== 'Paid', onClick: (r: unknown) => markPaid(r) },
  { label: '✓ Mark Paid', icon: '💰', show: (r: any) => String(r.status ?? '') !== 'Paid', onClick: (r: unknown) => markPaid(r) },
...
<template>
  <div class="fade-in">
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px">
      <span class="page-title">Contractor Payments</span>
