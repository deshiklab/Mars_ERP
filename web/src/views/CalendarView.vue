...
onMounted(async () => {
  ...
  if (r.ok && r.data) {
    const arr = r.data.collections['calendar_events']
...
const stats = computed(() => [
  ...
  { label: 'Upcoming', value: String(rows.value.filter((e: any) => e.status === 'Upcoming' || e.status === 'Scheduled').length), color: '#2e7d32' },
  { label: 'Done', value: String(rows.value.filter((e: any) => e.status === 'Done' || e.status === 'Completed').length), color: '#1565c0' }
])
...
const columns = computed<TableColumn<any>[]>(() => [
  ...
  },
  {
    ...
    label: 'Type',
    sortable: false,
    renderHtml: (x) => `<span class='pill' style='background:#f3e5f5;color:#7b1fa2'>${esc(x.type||'—')}</span>`
  },
  {
    key: 'status',
    label: 'Status',
...
<template>
  <div class="fade-in">
    ...
    <GenericDetailDrawer :record="detailRec" :title="'Calendar'" @close="detailRec = null" :records="detailList" />
</template>
