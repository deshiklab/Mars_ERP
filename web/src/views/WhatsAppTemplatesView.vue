...
onMounted(async () => {
  ...
  if (r.ok && r.data) {
    const arr = r.data.collections['whatsapp_templates']
...
const stats = computed(() => [
  { label: 'Templates', value: String(rows.value.length), color: '#2f80ed' },
  { label: 'Active', value: String(rows.value.filter((t: any) => t.active).length), color: '#2e7d32' },
  { label: 'Modules', value: String(new Set(rows.value.map((t: any) => t.module)).size), color: '#1565c0' }
])
...
const columns = computed<TableColumn<any>[]>(() => [
  {
    ...
    label: 'Template',
    sortable: false,
    renderHtml: (x) => `<div style='font-weight:500;color:#333'>${esc(x.name)}</div><div style='font-size:9px;color:#888'>${esc(x.event||'')}</div>`
  },
  {
    key: 'module',
    label: 'Module',
    sortable: false,
    renderHtml: (x) => `<span class='pill' style='background:#f0f4ff;color:#2f80ed'>${esc(x.module||'—')}</span>`
  },
  {
    key: 'body',
    label: 'Body',
    sortable: false,
    renderHtml: (x) => `<span style='font-size:9px;color:#888;display:block;max-width:300px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap'>${esc(x.body||'')}</span>`
  },
  {
    key: 'active',
    label: 'Active',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:12px'>${x.active ? '✅' : '⭕'}</span>`
  },])
...
<template>
  <div class="fade-in">
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px">
      <span class="page-title">WhatsApp Templates</span>
    ...
    <GenericDetailDrawer :record="detailRec" :title="'WhatsApp Templates'" @close="detailRec = null" :records="detailList" />
</template>
