...
onMounted(async () => {
  ...
  if (r.ok && r.data) {
    const arr = r.data.collections['tasks']
...
const stats = computed(() => [
  { label: 'Tasks', value: String(rows.value.length), color: '#2f80ed' },
  { label: 'To Do', value: String(rows.value.filter((t: any) => t.status === 'To Do').length), color: '#e65100' },
...
const columns = computed<TableColumn<any>[]>(() => [
  {
    ...
    label: 'Task',
    sortable: false,
    renderHtml: (x) => `<div style='font-weight:500;color:#333'>${esc(x.title)}</div><div style='font-size:9px;color:#888'>${esc(x.project||'')} · ${esc(x.desc||'')}</div>`
  },
  {
    key: 'assignee',
    label: 'Assignee',
    sortable: false,
    renderHtml: (x) => `<span style='font-size:10px;color:#555'>${esc(x.assignee||'—')}</span>`
  },
  {
    key: 'priority',
    label: 'Priority',
  ...
  },
  {
    ...
    label: 'Deadline',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:10px;color:${x.deadline < '2026-08-10' ? '#c62828' : '#555'}'>${esc(x.deadline||'—')}</span>`
  },
  {
    key: 'tags',
    label: 'Tags',
    sortable: false,
    renderHtml: (x) => `<span style='display:inline-flex;gap:3px;flex-wrap:wrap'>${(x.tags||[]).map((t: string) => `<span style='background:#f0f4ff;color:#2f80ed;border-radius:6px;padding:0 5px;font-size:8px'>${esc(t)}</span>`).join('')}</span>`
  },
  {
    key: 'status',
    label: 'Status',
...
<template>
  <div class="fade-in">
    ...
    <GenericDetailDrawer :record="detailRec" :title="'Tasks'" @close="detailRec = null" :records="detailList" />
</template>
