...
async function setCompleteStatus(r: unknown, st: string) {
  ...
  try {
    ...
    const res = await api.sync({ compliance: rows })
...
onMounted(async () => {
  ...
  if (r.ok && r.data) {
    const arr = r.data.collections['compliance']
...
const stats = computed(() => [
  ...
  { label: 'Due Soon', value: String(rows.value.filter((c: any) => c.status === 'Due').length), color: '#e65100' },
  { label: 'Compliant', value: String(rows.value.filter((c: any) => c.status === 'Compliant' || c.status === 'Renewed').length), color: '#2e7d32' }
])
...
const columns = computed<TableColumn<any>[]>(() => [
  ...
  },
  {
    ...
    label: 'Due',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:10px;color:${x.due < '2026-08-10' ? '#c62828' : '#555'}'>${esc(x.due||'—')}</span>`
  },
  {
    key: 'renewal',
    label: 'Renewal',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:10px;color:#555'>${esc(x.renewal||'—')}</span>`
  },
  {
    key: 'priority',
    label: 'Priority',
...
<template>
  <div class="fade-in">
    ...
    <DataTable
      ...
      search-placeholder="Search compliance…"
    />
  </div>
    <GenericDetailDrawer :record="detailRec" :title="'Compliance'" @close="detailRec = null" :records="detailList" />
</template>
