...
async function setCompleteStatus(r: unknown, st: string) {
  const id = String((r as any)?.id ?? '')
  if (!id || actBusy.value) return
  actBusy.value = true
  try {
    const rows = [...items.value].map((x: any) => (String(x.id ?? '') === id ? { ...x, status: st } : x))
    const res = await api.sync({ maintenanceLog: rows })
    if (res.ok) { items.value = rows; showToast('Complete applied') }
...
onMounted(async () => {
  ...
  if (r.ok && r.data) {
    const arr = r.data.collections['maintenanceLog']
...
const stats = computed(() => [
  ...
  { label: 'Open', value: String(rows.value.filter((m: any) => m.status === 'Open' || m.status === 'Pending').length), color: '#e65100' },
  { label: 'Total Cost', value: String(bdt(rows.value.reduce((s: number, m: any) => s + (m.cost ?? 0), 0))), color: '#c62828' }
])
...
const columns = computed<TableColumn<any>[]>(() => [
  ...
  },
  {
    ...
    label: 'Equipment',
    sortable: false,
    renderHtml: (x) => `<div style='font-weight:500;color:#333'>${esc(x.equipment)}</div><div style='font-size:9px;color:#888'>${esc(x.type||'')}</div>`
  },
  {
    key: 'date',
    label: 'Date',
  ...
  },
  {
    ...
    label: 'Notes',
    sortable: false,
    renderHtml: (x) => `<span style='font-size:9px;color:#888;display:block;max-width:260px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap'>${esc(x.notes||'')}</span>`
  },
  {
    key: 'status',
    label: 'Status',
...
const actions = computed(() => [
  { label: '✓ Complete', icon: '✅', show: (r: any) => !['Completed','Done'].includes(r.status), onClick: (r: any) => setCompleteStatus(r, 'Completed') },
...
<template>
  <div class="fade-in">
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px">
      <span class="page-title">Maintenance Log</span>
    ...
    <GenericDetailDrawer :record="detailRec" :title="'Maintenance Log'" @close="detailRec = null" :records="detailList" />
</template>
