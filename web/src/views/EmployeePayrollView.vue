...
onMounted(async () => {
  ...
  if (r.ok && r.data) {
    const arr = r.data.collections['hr_payroll']
...
const stats = computed(() => [
  { label: 'Records', value: String(rows.value.length), color: '#2f80ed' },
  { label: 'Net Total', value: String(bdt(rows.value.reduce((s: number, p: any) => s + (p.net ?? 0), 0))), color: '#2e7d32' },
  { label: 'Paid', value: String(rows.value.filter((p: any) => p.status === 'Paid').length), color: '#1565c0' }
])
...
const columns = computed<TableColumn<any>[]>(() => [
  ...
  },
  {
    ...
    label: 'Employee',
    sortable: false,
    renderHtml: (x) => `<div style='font-weight:500;color:#333'>${esc(x.employeeId)}</div><div style='font-size:9px;color:#888'>${esc(x.month||'')}</div>`
  },
  {
    key: 'basic',
    label: 'Basic',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:10px;color:#333'>${bdt(x.basic)}</span>`
  },
  {
    key: 'allowance',
    label: 'Allowance',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:10px;color:#555'>${bdt(x.allowance)}</span>`
  },
  {
    key: 'bonus',
    label: 'Bonus',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:10px;color:#2e7d32;font-weight:600'>${bdt(x.bonus)}</span>`
  },
  {
    key: 'deduction',
    label: 'Deduction',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:10px;color:#c62828'>-${bdt(x.deduction)}</span>`
  },
  {
    key: 'net',
    label: 'Net',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:11px;color:#2e7d32;font-weight:700'>${bdt(x.net)}</span>`
  },
  {
    key: 'status',
    label: 'Status',
...
<template>
  <div class="fade-in">
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px">
      <span class="page-title">Employee Payroll</span>
    ...
    <GenericDetailDrawer :record="detailRec" :title="'Employee Payroll'" @close="detailRec = null" :records="detailList" />
</template>
