...
onMounted(async () => {
  ...
  if (r.ok && r.data) {
    const arr = r.data.collections['audits']
...
const stats = computed(() => [
  { label: 'Audits', value: String(rows.value.length), color: '#2f80ed' },
  { label: 'Passed', value: String(rows.value.filter((a: any) => (a.status === 'Completed' || a.status === 'Passed') && (a.score ?? 0) >= 80).length), color: '#2e7d32' },
  { label: 'Failed', value: String(rows.value.filter((a: any) => a.status === 'Failed').length), color: '#c62828' }
])
...
const columns = computed<TableColumn<any>[]>(() => [
  ...
  },
  {
    ...
    label: 'Auditor',
    sortable: false,
    renderHtml: (x) => `<span style='font-size:10px;color:#555'>${esc(x.auditor||'—')}</span>`
  },
  {
    key: 'date',
    label: 'Date',
  ...
  },
  {
    ...
    label: 'Score',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:10px;color:${x.score >= 80 ? '#2e7d32' : x.score >= 60 ? '#e65100' : '#c62828'};font-weight:700'>${esc(x.score??'—')}</span>`
  },
  {
    key: 'status',
    label: 'Status',
...
<template>
  <div class="fade-in">
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px">
      <span class="page-title">QC Audits</span>
    ...
    <GenericDetailDrawer :record="detailRec" :title="'QC Audits'" @close="detailRec = null" :records="detailList" />
</template>
