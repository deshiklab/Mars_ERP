...
async function setTransfersStatus(r: unknown) {
  const id = String((r as any).id ?? '')
  const rows = [...items.value]
  const idx = rows.findIndex((x: any) => String(x.id ?? '') === id)
  if (idx < 0) return
  rows[idx] = { ...(rows[idx] as any), status: 'Completed' }
  const res = await api.sync({ transfers: rows })
  if (res.ok) {
...
onMounted(async () => {
  ...
  if (r.ok && r.data) {
    const arr = r.data.collections['transfers']
...
const stats = computed(() => [
  { label: 'Transfers', value: String(rows.value.length), color: '#2f80ed' },
  { label: 'Total', value: String(bdt(rows.value.reduce((s: number, t: any) => s + (t.amount ?? 0), 0))), color: '#1565c0' },
  { label: 'Completed', value: String(rows.value.filter((t: any) => t.status === 'Completed').length), color: '#2e7d32' }
])
...
const columns = computed<TableColumn<any>[]>(() => [
  ...
  },
  {
    ...
    label: 'Note',
    sortable: false,
    renderHtml: (x) => `<span style='font-size:9px;color:#888'>${esc(x.note||'—')}</span>`
  },
  {
    key: 'status',
    label: 'Status',
...
const actions = computed(() => [
  { label: '✓ Complete', icon: '🏦', show: (r: any) => !['Completed'].includes(String(r.status ?? '')), onClick: (r: unknown) => setTransfersStatus(r) },
...
<template>
  <div class="fade-in">
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px">
      <span class="page-title">Transfers</span>
    ...
    <GenericDetailDrawer :record="detailRec" :title="'Transfers'" @close="detailRec = null" :records="detailList" />
</template>
