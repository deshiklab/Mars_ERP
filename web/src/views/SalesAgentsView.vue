...
onMounted(async () => {
  ...
  if (r.ok && r.data) {
    const arr = r.data.collections['sales_agents']
...
const stats = computed(() => [
  { label: 'Agents', value: String(rows.value.length), color: '#2f80ed' },
  { label: 'Active', value: String(rows.value.filter((a: any) => a.status === 'Active').length), color: '#2e7d32' },
  { label: 'Total Leads', value: String(rows.value.reduce((s: number, a: any) => s + (a.leads ?? 0), 0)), color: '#1565c0' }
])
...
const columns = computed<TableColumn<any>[]>(() => [
  ...
  },
  {
    ...
    label: 'Deals',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:10px;color:#2e7d32;font-weight:600'>${esc(x.deals??0)}</span>`
  },
  {
    key: 'conversions',
    label: 'Conversions',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:10px;color:#555'>${esc(x.conversions??'—')}%</span>`
  },
  {
    key: 'commission',
    label: 'Commission',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:10px;color:#555'>${esc(x.commission??'—')}%</span>`
  },
  {
    key: 'rating',
    label: 'Rating',
...
<template>
  <div class="fade-in">
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px">
      <span class="page-title">Sales Agents</span>
    ...
    <GenericDetailDrawer :record="detailRec" :title="'Sales Agents'" @close="detailRec = null" :records="detailList" />
</template>
