...
async function setReconItemsStatus(r: unknown) {
  const id = String((r as any).id ?? '')
  const rows = [...items.value]
  const idx = rows.findIndex((x: any) => String(x.id ?? '') === id)
  if (idx < 0) return
  rows[idx] = { ...(rows[idx] as any), status: 'Matched' }
  const res = await api.sync({ recon_items: rows })
  if (res.ok) {
    showToast(`Item matched`)
...
onMounted(async () => {
  ...
  if (r.ok && r.data) {
    const arr = r.data.collections['recon_items']
...
const stats = computed(() => [
  ...
  { label: 'Matched', value: String(rows.value.filter((r: any) => r.status === 'Matched').length), color: '#2e7d32' },
  { label: 'Unmatched', value: String(rows.value.filter((r: any) => r.status === 'Unmatched' || r.status === 'Pending').length), color: '#e65100' }
])
...
const columns = computed<TableColumn<any>[]>(() => [
  ...
  },
  {
    ...
    label: 'Description',
    sortable: false,
    renderHtml: (x) => `<div style='font-weight:500;color:#333'>${esc(x.desc||'—')}</div><div style='font-size:9px;color:#888'>${esc(x.ref||'')}</div>`
  },
  {
    key: 'date',
    label: 'Date',
  ...
  },
  {
    ...
    label: 'Matched To',
    sortable: false,
    renderHtml: (x) => `<span style='font-size:9px;color:#888'>${esc(x.matchedTo||'—')}</span>`
  },
  {
    key: 'status',
    label: 'Status',
...
const actions = computed(() => [
  { label: '✓ Mark Matched', icon: '🔗', show: (r: any) => !['Matched'].includes(String(r.status ?? '')), onClick: (r: unknown) => setReconItemsStatus(r) },
...
<template>
  <div class="fade-in">
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px">
      <span class="page-title">Reconciliation</span>
    ...
    <DataTable
      ...
      search-placeholder="Search recon items…"
    />
  </div>
    <GenericDetailDrawer :record="detailRec" :title="'Reconciliation'" @close="detailRec = null" :records="detailList" />
</template>
