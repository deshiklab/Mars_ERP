...
const stats = computed(() => [
  { label: 'Work Orders', value: String(data.workOrders.length), color: '#2f80ed' },
  { label: 'In Progress', value: String(data.workOrders.filter(w=>w.status==='In Progress').length), color: '#e65100' },
...
const columns = computed<TableColumn<any>[]>(() => [
  ...
  },
  {
    ...
    label: 'Title',
    sortable: false,
    renderHtml: (x) => `<div style='font-weight:500;color:#333'>${esc(x.title||x.name||'')}</div>`
  },
  {
    key: 'project',
    label: 'Project',
  ...
  },
  {
    ...
    label: 'Assignee',
    sortable: false,
    renderHtml: (x) => `<span style='font-size:10px;color:#555'>${esc(x.assignee||x.assignedTo||'—')}</span>`
  },
  {
    key: 'status',
    label: 'Status',
...
<template>
  <div class="fade-in">
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px">
      <span class="page-title">Work Orders</span>
      <span class="page-subtitle">{{ data.workOrders.length }} records</span>
    </div>
    ...
    <GenericDetailDrawer :record="detailRec" :title="'Work Orders'" @close="detailRec = null" :records="detailList" />
</template>
