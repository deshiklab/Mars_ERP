...
onMounted(async () => {
  ...
  if (r.ok && r.data) {
    const arr = r.data.collections['whatsapp_broadcasts']
...
const stats = computed(() => [
  { label: 'Broadcasts', value: String(rows.value.length), color: '#2f80ed' },
  { label: 'Delivered', value: String(rows.value.reduce((s: number, w: any) => s + (w.delivered ?? 0), 0)), color: '#2e7d32' },
  { label: 'Recipients', value: String(rows.value.reduce((s: number, w: any) => s + (w.recipients ?? 0), 0)), color: '#1565c0' }
])
...
const columns = computed<TableColumn<any>[]>(() => [
  ...
  },
  {
    ...
    label: 'Broadcast',
    sortable: false,
    renderHtml: (x) => `<div style='font-weight:500;color:#333'>${esc(x.name)}</div><div style='font-size:9px;color:#888'>${esc(x.template||'')} · ${esc(x.segment||'')}</div>`
  },
  {
    key: 'time',
    label: 'Sent',
  ...
  },
  {
    ...
    label: 'Recipients',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:10px;color:#333'>${esc(x.recipients??0)}</span>`
  },
  {
    key: 'delivered',
    label: 'Delivered',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:10px;color:#2e7d32;font-weight:600'>${esc(x.delivered??0)}</span>`
  },
  {
    key: 'failed',
    label: 'Failed',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:10px;color:${x.failed > 0 ? '#c62828' : '#888'};font-weight:600'>${esc(x.failed??0)}</span>`
  },
  {
    key: 'status',
    label: 'Status',
...
<template>
  <div class="fade-in">
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px">
      <span class="page-title">WhatsApp Engine</span>
    ...
    <GenericDetailDrawer :record="detailRec" :title="'WhatsApp Engine'" @close="detailRec = null" :records="detailList" />
</template>
