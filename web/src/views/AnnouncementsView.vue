...
async function setPublishStatus(r: unknown, st: string) {
  const id = String((r as any)?.id ?? '')
  if (!id || actBusy.value) return
  actBusy.value = true
  try {
    const rows = [...items.value].map((x: any) => (String(x.id ?? '') === id ? { ...x, status: st } : x))
    const res = await api.sync({ announcements: rows })
    if (res.ok) { items.value = rows; showToast('Publish applied') }
...
onMounted(async () => {
  ...
  if (r.ok && r.data) {
    const arr = r.data.collections['announcements']
...
const stats = computed(() => [
  { label: 'Announcements', value: String(rows.value.length), color: '#2f80ed' },
  { label: 'Pinned', value: String(rows.value.filter((a: any) => a.pinned).length), color: '#e65100' },
  { label: 'Total Views', value: String(rows.value.reduce((s: number, a: any) => s + (a.views ?? 0), 0)), color: '#1565c0' }
])
...
const columns = computed<TableColumn<any>[]>(() => [
  {
    ...
    label: 'Announcement',
    sortable: false,
    renderHtml: (x) => `<div style='font-weight:500;color:#333'>${x.pinned ? '📌 ' : ''}${esc(x.title)}</div><div style='font-size:9px;color:#888'>${esc(x.author||'')} · ${esc(x.date||'')}</div>`
  },
  {
    key: 'body',
    label: 'Message',
    sortable: false,
    renderHtml: (x) => `<span style='font-size:10px;color:#555;display:block;max-width:300px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap'>${esc(x.body||'')}</span>`
  },
  {
    key: 'audience',
    label: 'Audience',
    sortable: false,
    renderHtml: (x) => `<span class='pill' style='background:#f0f4ff;color:#2f80ed'>${esc(x.audience||'—')}</span>`
  },
  {
    key: 'priority',
    label: 'Priority',
  ...
  },
  {
    ...
    label: 'Views',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:10px;color:#333'>${esc(x.views??0)}</span>`
  },])
const actions = computed(() => [
  { label: '✓ Publish', icon: '✅', show: (r: any) => !['Published'].includes(r.status), onClick: (r: any) => setPublishStatus(r, 'Published') },
...
<template>
  <div class="fade-in">
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px">
      <span class="page-title">Announcements</span>
    ...
    <GenericDetailDrawer :record="detailRec" :title="'Announcements'" @close="detailRec = null" :records="detailList" />
</template>
