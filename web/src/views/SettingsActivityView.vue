...
onMounted(async () => {
  ...
  if (r.ok && r.data) {
    const arr = r.data.collections['settings_activity']
...
const stats = computed(() => [
  { label: 'Events', value: String(rows.value.length), color: '#2f80ed' },
  { label: 'Users', value: String(new Set(rows.value.map((s: any) => s.user)).size), color: '#1565c0' },
  { label: 'Recent', value: String(rows.value.filter((s: any) => String(s.date || '').startsWith('2026-08')).length), color: '#2e7d32' }
])
...
const columns = computed<TableColumn<any>[]>(() => [
  {
    ...
    label: 'When',
    sortable: true,
    renderHtml: (x) => `<div style='font-size:9px;color:#888'>${esc(x.date||'—')}</div>`
  },
  {
    key: 'user',
    label: 'User',
  ...
  },
  {
    ...
    label: 'Action',
    sortable: false,
    renderHtml: (x) => `<div style='font-weight:500;color:#333'>${esc(x.action||'—')}</div>`
  },
  {
    key: 'detail',
    label: 'Detail',
    sortable: false,
    renderHtml: (x) => `<span style='font-size:9px;color:#888'>${esc(x.detail||'')}</span>`
  },
  {
    key: 'ip',
    label: 'IP',
    sortable: false,
    renderHtml: (x) => `<span style='font-size:9px;color:#888'>${esc(x.ip||'—')}</span>`
  },])
...
<template>
  <div class="fade-in">
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px">
      <span class="page-title">Settings Activity</span>
    ...
    <GenericDetailDrawer :record="detailRec" :title="'Settings Activity'" @close="detailRec = null" :records="detailList" />
</template>
