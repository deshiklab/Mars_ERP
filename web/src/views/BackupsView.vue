...
async function setCompleteStatus(r: unknown, st: string) {
  ...
  try {
    ...
    const res = await api.sync({ backups: rows })
...
onMounted(async () => {
  ...
  if (r.ok && r.data) {
    const arr = r.data.collections['backups']
...
const stats = computed(() => [
  { label: 'Backups', value: String(rows.value.length), color: '#2f80ed' },
  { label: 'Successful', value: String(rows.value.filter((b: any) => b.status === 'Success' || b.status === 'Completed').length), color: '#2e7d32' },
  { label: 'Total Size', value: String(rows.value.reduce((s: number, b: any) => s + (b.sizeBytes ?? 0), 0) >= 1073741824 ? (rows.value.reduce((s: number, b: any) => s + (b.sizeBytes ?? 0), 0) / 1073741824).toFixed(2) + ' GB' : (rows.value.reduce((s: number, b: any) => s + (b.sizeBytes ?? 0), 0) / 1048576).toFixed(1) + ' MB'), color: '#1565c0' }
])
...
const columns = computed<TableColumn<any>[]>(() => [
  ...
  },
  {
    ...
    label: 'Size',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:10px;color:#333'>${esc(x.size||'—')}</span>`
  },
  {
    key: 'included',
    label: 'Included',
    sortable: false,
    renderHtml: (x) => `<span style='font-size:9px;color:#888'>${esc(x.included||'—')}</span>`
  },
  {
    key: 'status',
    label: 'Status',
...
<template>
  <div class="fade-in">
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px">
      <span class="page-title">Data Backup</span>
    ...
    <GenericDetailDrawer :record="detailRec" :title="'Data Backup'" @close="detailRec = null" :records="detailList" />
</template>
