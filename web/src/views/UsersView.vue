...
async function setDeactivateStatus(r: unknown, st: string) {
  const id = String((r as any)?.id ?? '')
  if (!id || actBusy.value) return
  actBusy.value = true
  try {
    const rows = [...items.value].map((x: any) => (String(x.id ?? '') === id ? { ...x, status: st } : x))
    const res = await api.sync({ users: rows })
    if (res.ok) { items.value = rows; showToast('Deactivate applied') }
...
onMounted(async () => {
  ...
  if (r.ok && r.data) {
    const arr = r.data.collections['users']
...
const stats = computed(() => [
  { label: 'Users', value: String(rows.value.length), color: '#2f80ed' },
  { label: 'Active', value: String(rows.value.filter((u: any) => u.status === 'Active').length), color: '#2e7d32' },
  { label: 'Roles', value: String(new Set(rows.value.map((u: any) => u.role)).size), color: '#1565c0' }
])
...
const columns = computed<TableColumn<any>[]>(() => [
  {
    ...
    label: 'User',
    sortable: false,
    renderHtml: (x) => `<div style='display:flex;align-items:center;gap:8px'><span style='width:26px;height:26px;border-radius:50%;background:linear-gradient(135deg,#7b1fa2,#2f80ed);color:#fff;display:inline-flex;align-items:center;justify-content:center;font-size:10px;font-weight:700'>${esc(x.initials||x.name.slice(0,2).toUpperCase())}</span><div><div style='font-weight:500;color:#333'>${esc(x.name)}</div><div style='font-size:9px;color:#888'>${esc(x.email||'')}</div></div></div>`
  },
  {
    key: 'role',
    label: 'Role',
    sortable: true,
    renderHtml: (x) => `<span class='pill' style='background:#f3e5f5;color:#7b1fa2'>${esc(x.role||'—')}</span>`
  },
  {
    key: 'dept',
    label: 'Department',
    sortable: false,
    renderHtml: (x) => `<span style='font-size:10px;color:#555'>${esc(x.dept||'—')}</span>`
  },
  {
    key: 'status',
    label: 'Status',
...
const actions = computed(() => [
  { label: '✓ Deactivate', icon: '✅', show: (r: any) => r.status === 'Active', onClick: (r: any) => setDeactivateStatus(r, 'Inactive') },
...
<template>
  <div class="fade-in">
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px">
      <span class="page-title">User & Role Mgmt</span>
    ...
    <GenericDetailDrawer :record="detailRec" :title="'User & Role Mgmt'" @close="detailRec = null" :records="detailList" />
</template>
