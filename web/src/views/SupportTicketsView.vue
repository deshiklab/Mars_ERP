...
onMounted(async () => {
  ...
  if (r.ok && r.data) {
    const arr = r.data.collections['supportTickets']
...
const stats = computed(() => [
  { label: 'Tickets', value: String(rows.value.length), color: '#2f80ed' },
  { label: 'Open', value: String(rows.value.filter((t: any) => t.status === 'Open').length), color: '#e65100' },
  { label: 'Resolved', value: String(rows.value.filter((t: any) => t.status === 'Resolved' || t.status === 'Closed').length), color: '#2e7d32' }
])
...
const columns = computed<TableColumn<any>[]>(() => [
  ...
  },
  {
    ...
    label: 'Subject',
    sortable: false,
    renderHtml: (x) => `<div style='font-weight:500;color:#333'>${esc(x.subject)}</div><div style='font-size:9px;color:#888'>${esc(x.category||'')}</div>`
  },
  {
    key: 'customerName',
    label: 'Customer',
    sortable: false,
    renderHtml: (x) => `<span style='font-size:10px;color:#555'>${esc(x.customerName||'—')}</span>`
  },
  {
    key: 'priority',
    label: 'Priority',
...
<template>
  <div class="fade-in">
    ...
    <GenericDetailDrawer :record="detailRec" :title="'Support Tickets'" @close="detailRec = null" :records="detailList" />
</template>
