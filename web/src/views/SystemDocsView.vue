...
onMounted(async () => {
  ...
  if (r.ok && r.data) {
    const arr = r.data.collections['systemDocs']
...
const stats = computed(() => [
  ...
  { label: 'Categories', value: String(new Set(rows.value.map((d: any) => d.category)).size), color: '#1565c0' },
  { label: 'Total Read Time', value: String(rows.value.reduce((s: number, d: any) => s + (d.readTime ?? 0), 0)), color: '#2e7d32' }
])
...
const columns = computed<TableColumn<any>[]>(() => [
  {
    ...
    label: 'Document',
    sortable: false,
    renderHtml: (x) => `<div style='font-weight:500;color:#333'>${esc(x.title)}</div><div style='font-size:9px;color:#888'>${esc(x.excerpt||'')}</div>`
  },
  {
    key: 'category',
    label: 'Category',
  ...
  },
  {
    ...
    label: 'Read Time',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:10px;color:#555'>${esc(x.readTime??'—')} min</span>`
  },
  {
    key: 'updated',
    label: 'Updated',
...
<template>
  <div class="fade-in">
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px">
      <span class="page-title">System Manual</span>
    ...
    <GenericDetailDrawer :record="detailRec" :title="'System Manual'" @close="detailRec = null" :records="detailList" />
</template>
