...
const stats = computed(() => [
  { label: 'BOQ Lines', value: String(data.boq.length), color: '#2f80ed' },
  { label: 'Approved', value: String(data.boq.filter(b=>b.status==='Approved').length), color: '#2e7d32' },
  { label: 'Pending', value: String(data.boq.filter(b=>b.status!=='Approved').length), color: '#ff8f00' }
])
...
const columns = computed<TableColumn<any>[]>(() => [
  {
    ...
    label: 'Item',
    sortable: false,
    renderHtml: (x) => `<div style='font-weight:500;color:#333'>${esc(x.item)}</div><div style='font-size:9px;color:#888'>${esc(x.category||'')} · ${esc(x.project||'')}</div>`
  },
  {
    key: 'qty',
    label: 'Qty',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:10px;color:#333'>${esc(x.qty??'—')} ${esc(x.unit||'')}</span>`
  },
  {
    key: 'rate',
    label: 'Rate',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:10px;color:#555'>${bdt(x.rate)}</span>`
  },
  {
    key: 'status',
    label: 'Status',
...
<template>
  <div class="fade-in">
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px">
      <span class="page-title">BOQ & Cost Control</span>
      <span class="page-subtitle">{{ data.boq.length }} records</span>
    </div>
    ...
    <GenericDetailDrawer :record="detailRec" :title="'BOQ & Cost Control'" @close="detailRec = null" :records="detailList" />
</template>
