...
async function setResolveStatus(r: unknown, st: string) {
  const id = String((r as any)?.id ?? '')
  if (!id || actBusy.value) return
  actBusy.value = true
  try {
    const rows = [...items.value].map((x: any) => (String(x.id ?? '') === id ? { ...x, status: st } : x))
    const res = await api.sync({ afterSales: rows })
    if (res.ok) { items.value = rows; showToast('Resolve applied') }
...
onMounted(async () => {
  ...
  if (r.ok && r.data) {
    const arr = r.data.collections['afterSales']
...
const stats = computed(() => [
  { label: 'Requests', value: String(rows.value.length), color: '#2f80ed' },
  { label: 'In Progress', value: String(rows.value.filter((a: any) => a.status === 'In Progress').length), color: '#e65100' },
  { label: 'Completed', value: String(rows.value.filter((a: any) => a.status === 'Completed').length), color: '#2e7d32' }
])
...
const columns = computed<TableColumn<any>[]>(() => [
  ...
  },
  {
    ...
    label: 'Customer',
    sortable: false,
    renderHtml: (x) => `<div style='font-weight:500;color:#333'>${esc(x.customer)}</div><div style='font-size:9px;color:#888'>${esc(x.unit||'')}</div>`
  },
  {
    key: 'serviceType',
    label: 'Service',
    sortable: false,
    renderHtml: (x) => `<span class='pill' style='background:#f0f4ff;color:#2f80ed'>${esc(x.serviceType||'—')}</span>`
  },
  {
    key: 'date',
    label: 'Date',
  ...
  },
  {
    ...
    label: 'Assigned',
    sortable: false,
    renderHtml: (x) => `<span style='font-size:10px;color:#555'>${esc(x.assignedTo||'—')}</span>`
  },
  {
    key: 'cost',
    label: 'Cost',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:10px;color:#c62828;font-weight:600'>${bdt(x.cost)}</span>`
  },
  {
    key: 'status',
    label: 'Status',
...
const actions = computed(() => [
  { label: '✓ Resolve', icon: '✅', show: (r: any) => !['Resolved','Completed','Done'].includes(r.status), onClick: (r: any) => setResolveStatus(r, 'Resolved') },
...
<template>
  <div class="fade-in">
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px">
      <span class="page-title">After-Sales Service</span>
    ...
    <DataTable
      ...
      search-placeholder="Search after-sales…"
    />
  </div>
    <GenericDetailDrawer :record="detailRec" :title="'After-Sales Service'" @close="detailRec = null" :records="detailList" />
</template>
