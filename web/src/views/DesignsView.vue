...
onMounted(async () => {
  ...
  if (r.ok && r.data) {
    const arr = r.data.collections['designs']
...
const stats = computed(() => [
  { label: 'Designs', value: String(rows.value.length), color: '#2f80ed' },
  { label: 'Approved', value: String(rows.value.filter((d: any) => d.approved).length), color: '#2e7d32' },
  { label: 'Drafts', value: String(rows.value.filter((d: any) => !d.approved && d.status === 'Draft').length), color: '#ff8f00' }
])
...
const columns = computed<TableColumn<any>[]>(() => [
  {
    ...
    label: 'Design',
    sortable: false,
    renderHtml: (x) => `<div style='font-weight:500;color:#333'>${esc(x.name)}</div><div style='font-size:9px;color:#888'>${esc(x.type||'')} v${esc(x.version??'1')}</div>`
  },
  {
    key: 'project',
    label: 'Project',
  ...
  },
  {
    ...
    label: 'Designer',
    sortable: false,
    renderHtml: (x) => `<span style='font-size:10px;color:#555'>${esc(x.designer||'—')}</span>`
  },
  {
    key: 'submitted',
    label: 'Submitted',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:10px;color:#555'>${esc(x.submitted||'—')}</span>`
  },
  {
    key: 'status',
    label: 'Status',
...
<template>
  <div class="fade-in">
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px">
      <span class="page-title">Designs</span>
    ...
    <GenericDetailDrawer :record="detailRec" :title="'Designs'" @close="detailRec = null" :records="detailList" />
</template>
