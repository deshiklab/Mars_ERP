...
async function setShortlistStatus(r: unknown, st: string) {
  const id = String((r as any)?.id ?? '')
  if (!id || actBusy.value) return
  actBusy.value = true
  try {
    const rows = [...items.value].map((x: any) => (String(x.id ?? '') === id ? { ...x, status: st } : x))
    const res = await api.sync({ applicants: rows })
    if (res.ok) { items.value = rows; showToast('Shortlist applied') }
...
onMounted(async () => {
  ...
  if (r.ok && r.data) {
    const arr = r.data.collections['applicants']
...
const stats = computed(() => [
  { label: 'Applicants', value: String(rows.value.length), color: '#2f80ed' },
  { label: 'In Review', value: String(rows.value.filter((a: any) => a.status === 'In Review' || a.stage === 'Screening').length), color: '#ff8f00' },
  { label: 'Hired', value: String(rows.value.filter((a: any) => a.status === 'Hired').length), color: '#2e7d32' }
])
...
const columns = computed<TableColumn<any>[]>(() => [
  {
    ...
    label: 'Applicant',
    sortable: false,
    renderHtml: (x) => `<div style='font-weight:500;color:#333'>${esc(x.name)}</div><div style='font-size:9px;color:#888'>${esc(x.phone||'')} · ${esc(x.jobId||'')}</div>`
  },
  {
    key: 'applied',
    label: 'Applied',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:10px;color:#555'>${esc(x.applied||'—')}</span>`
  },
  {
    key: 'rating',
    label: 'Rating',
  ...
  },
  {
    ...
    label: 'Stage',
    sortable: false,
    renderHtml: (x) => `<span class='pill' style='background:#f3e5f5;color:#7b1fa2'>${esc(x.stage||'—')}</span>`
  },
  {
    key: 'status',
    label: 'Status',
...
const actions = computed(() => [
  { label: '✓ Shortlist', icon: '✅', show: (r: any) => !['Shortlisted','Hired','Rejected'].includes(r.status), onClick: (r: any) => setShortlistStatus(r, 'Shortlisted') },
...
<template>
  <div class="fade-in">
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px">
      <span class="page-title">Job Applicants</span>
    ...
    <GenericDetailDrawer :record="detailRec" :title="'Job Applicants'" @close="detailRec = null" :records="detailList" />
</template>
