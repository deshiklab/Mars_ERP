...
async function setTaxEntriesStatus(r: unknown) {
  const id = String((r as any).id ?? '')
  const rows = [...items.value]
  const idx = rows.findIndex((x: any) => String(x.id ?? '') === id)
  if (idx < 0) return
  rows[idx] = { ...(rows[idx] as any), status: 'Filed' }
  const res = await api.sync({ tax_entries: rows })
  if (res.ok) {
    showToast(`Tax entry filed`)
...
onMounted(async () => {
  ...
  if (r.ok && r.data) {
    const arr = r.data.collections['tax_entries']
...
const stats = computed(() => [
  ...
  { label: 'Total', value: String(bdt(rows.value.reduce((s: number, t: any) => s + (t.amount ?? 0), 0))), color: '#c62828' },
  { label: 'Filed', value: String(rows.value.filter((t: any) => t.status === 'Filed' || t.status === 'Paid').length), color: '#2e7d32' }
])
...
const columns = computed<TableColumn<any>[]>(() => [
  ...
  },
  {
    ...
    label: 'Description',
    sortable: false,
    renderHtml: (x) => `<div style='font-weight:500;color:#333'>${esc(x.desc||'—')}</div><div style='font-size:9px;color:#888'>${esc(x.project||'')}</div>`
  },
  {
    key: 'type',
    label: 'Type',
  ...
  },
  {
    ...
    label: 'Amount',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:11px;color:#c62828;font-weight:700'>${bdt(x.amount)}</span>`
  },
  {
    key: 'status',
    label: 'Status',
...
const actions = computed(() => [
  { label: '✓ Mark Filed', icon: '📄', show: (r: any) => !['Filed','Paid'].includes(String(r.status ?? '')), onClick: (r: unknown) => setTaxEntriesStatus(r) },
...
<template>
  <div class="fade-in">
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px">
      <span class="page-title">Tax Entries</span>
    ...
    <DataTable
      ...
      search-placeholder="Search tax entries…"
    />
  </div>
    <GenericDetailDrawer :record="detailRec" :title="'Tax Entries'" @close="detailRec = null" :records="detailList" />
</template>
