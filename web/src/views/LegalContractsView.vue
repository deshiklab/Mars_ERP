...
async function setActivateStatus(r: unknown, st: string) {
  const id = String((r as any)?.id ?? '')
  if (!id || actBusy.value) return
  actBusy.value = true
  try {
    const rows = [...items.value].map((x: any) => (String(x.id ?? '') === id ? { ...x, status: st } : x))
    const res = await api.sync({ legalContracts: rows })
    if (res.ok) { items.value = rows; showToast('Activate applied') }
...
onMounted(async () => {
  ...
  if (r.ok && r.data) {
    const arr = r.data.collections['legalContracts']
...
const columns = computed<TableColumn<any>[]>(() => [
  {
    ...
    label: 'Contract',
    sortable: false,
    renderHtml: (x) => `<div style='font-weight:500;color:#333'>${esc(x.title)}</div><div style='font-size:9px;color:#888'>${esc(x.type||'')} · ${esc(x.project||'')}</div>`
  },
  {
    key: 'party',
    label: 'Party',
    sortable: false,
    renderHtml: (x) => `<span style='font-size:10px;color:#555'>${esc(x.party||'—')}</span>`
  },
  {
    key: 'value',
    label: 'Value',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:10px;color:#333;font-weight:600'>${bdt(x.value)}</span>`
  },
  {
    key: 'signedDate',
    label: 'Signed',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:10px;color:#555'>${esc(x.signedDate||'—')}</span>`
  },
  {
    key: 'expiry',
    label: 'Expiry',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:10px;color:${x.expiry < '2026-08-10' ? '#c62828' : '#555'}'>${esc(x.expiry||'—')}</span>`
  },
  {
    key: 'status',
    label: 'Status',
...
const actions = computed(() => [
  { label: '✓ Activate', icon: '✅', show: (r: any) => !['Active','Completed','Done'].includes(r.status), onClick: (r: any) => setActivateStatus(r, 'Active') },
...
<template>
  <div class="fade-in">
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px">
      <span class="page-title">Legal Contracts</span>
    ...
    <GenericDetailDrawer :record="detailRec" :title="'Legal Contracts'" @close="detailRec = null" :records="detailList" />
</template>
