...
async function setPostStatus(r: unknown, st: string) {
  const id = String((r as any)?.id ?? '')
  if (!id || actBusy.value) return
  actBusy.value = true
  try {
    const rows = [...items.value].map((x: any) => (String(x.id ?? '') === id ? { ...x, status: st } : x))
    const res = await api.sync({ opening_balances: rows })
    if (res.ok) { items.value = rows; showToast('Post applied') }
...
onMounted(async () => {
  ...
  if (r.ok && r.data) {
    const arr = r.data.collections['opening_balances']
...
const stats = computed(() => [
  { label: 'Accounts', value: String(rows.value.length), color: '#2f80ed' },
  { label: 'Assets', value: String(rows.value.filter((b: any) => b.type === 'Asset').length), color: '#2e7d32' },
  { label: 'Liabilities', value: String(rows.value.filter((b: any) => b.type === 'Liability').length), color: '#c62828' }
])
...
const columns = computed<TableColumn<any>[]>(() => [
  ...
  },
  {
    ...
    label: 'Type',
    sortable: true,
    renderHtml: (x) => `<span class='pill' style='background:${x.type === 'Asset' ? '#e8f5e9' : x.type === 'Liability' ? '#ffebee' : '#f0f4ff'};color:${x.type === 'Asset' ? '#2e7d32' : x.type === 'Liability' ? '#c62828' : '#2f80ed'}'>${esc(x.type||'—')}</span>`
  },
  {
    key: 'balance',
    label: 'Balance',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:11px;font-weight:700;color:${String(x.balance).includes('-') ? '#c62828' : '#2e7d32'}'>${esc(x.balance||'৳ 0')}</span>`
  },])
...
<template>
  <div class="fade-in">
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px">
      <span class="page-title">Opening Balances</span>
    ...
    <GenericDetailDrawer :record="detailRec" :title="'Opening Balances'" @close="detailRec = null" :records="detailList" />
</template>
