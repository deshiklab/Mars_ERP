...
onMounted(async () => {
  ...
  if (r.ok && r.data) {
    const arr = r.data.collections['customerDocs']
...
const stats = computed(() => [
  ...
  { label: 'Customers', value: String(new Set(rows.value.map((d: any) => d.customerId)).size), color: '#1565c0' },
...
const columns = computed<TableColumn<any>[]>(() => [
  ...
  },
  {
    ...
    label: 'Customer',
    sortable: false,
    renderHtml: (x) => `<div style='font-weight:500;color:#333'>${esc(x.customerName)}</div><div style='font-size:9px;color:#888'>${esc(x.customerId||'')}</div>`
  },
  {
    key: 'docType',
    label: 'Type',
    sortable: false,
    renderHtml: (x) => `<span class='pill' style='background:#f0f4ff;color:#2f80ed'>${esc(x.docType||'—')}</span>`
  },
  {
    key: 'project',
    label: 'Project',
...
<template>
  <div class="fade-in">
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px">
      <span class="page-title">Customer Documents</span>
    ...
    <GenericDetailDrawer :record="detailRec" :title="'Customer Documents'" @close="detailRec = null" :records="detailList" />
</template>
