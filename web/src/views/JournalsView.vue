...
onMounted(async () => {
  ...
  if (r.ok && r.data) {
    const arr = r.data.collections['journals']
...
const stats = computed(() => [
  { label: 'Journals', value: String(rows.value.length), color: '#2f80ed' },
  { label: 'Total Lines', value: String(rows.value.reduce((s: number, j: any) => s + (j.lines?.length ?? 0), 0)), color: '#1565c0' },
  { label: 'Recent', value: String(rows.value.filter((j: any) => String(j.date || '').startsWith('2026-08')).length), color: '#2e7d32' }
])
...
const columns = computed<TableColumn<any>[]>(() => [
  ...
  },
  {
    ...
    label: 'Description',
    sortable: false,
    renderHtml: (x) => `<div style='font-weight:500;color:#333'>${esc(x.desc||'—')}</div>`
  },
  {
    key: 'ref',
    label: 'Reference',
    sortable: false,
    renderHtml: (x) => `<span style='font-size:9px;color:#888'>${esc(x.ref||'—')}</span>`
  },
  {
    key: 'lines',
    label: 'Lines',
    sortable: true,
    renderHtml: (x) => `<span style='font-size:10px;color:#555'>${x.lines?.length ?? 0} entries</span>`
  },])
...
<template>
  <div class="fade-in">
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px">
      <span class="page-title">Journal Entries</span>
    ...
    <GenericDetailDrawer :record="detailRec" :title="'Journal Entries'" @close="detailRec = null" :records="detailList" />
</template>
