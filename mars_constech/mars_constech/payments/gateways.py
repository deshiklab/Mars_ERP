...
def settle_invoice(invoice_name, amount, method, reference):
	...
	(method, reference). Runs as Administrator so portal users (who have no
	Payment Entry permission) can still trigger the settlement."""
	...
	# portal users trigger this — run as Administrator for the PE creation
	user = frappe.session.user
	frappe.set_user("Administrator")
	try:
		# a draft invoice must be submitted before it can be settled
		inv = frappe.get_doc("Sales Invoice", invoice_name)
		if inv.docstatus == 0:
			inv.submit()
			inv.reload()
